(function($) {
    ...
    $(function() {
        $( '#doaction' ).on('click', function() {
            if( !confirm( i4t3strings.delete_confirm ) ) {
                return false;
            }
        });
        $( '.404-group-submit' ).on( 'click', function() {
            var group_by = $( this ).closest( '.bulkactions' ).find( '.404_group_by' ).val();
            if(group_by != '' && typeof group_by != 'undefined') {
                $( this ).closest( "form" ).attr( 'action', window.location.href + '&group_by_top='+group_by );
            }
        });
